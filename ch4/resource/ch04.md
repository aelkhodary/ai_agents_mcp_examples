...
## ----------------------------------------------------------------------
  ...
  ## The Logging Callback Flow Explained

  ```html
  ────────────────────────────────────────


  Step 1: Client Registers the Callback

  When client connects, it registers _handle_logs with ClientSession:

   ch4/01_handle_logging/client.py lines 78-85

    78 │# Start MCP client session
    79 │self._session = await self._exit_stack.enter_async_context(
    80 │    ClientSession(
    81 │        read_stream=self.read,
    82 │        write_stream=self.write,
    83 │        logging_callback=self._handle_logs,  # <-- REGISTERED HERE
    84 │    ),
    85 │)
  ...
  Step 2: Server Sends Log Message

  When any tool runs on the server, it can send log messages via ctx.info(), ctx.warning(), etc.:

   ch4/calculator_server.py lines 47-52

    47 │@mcp.tool()
    48 │async def add(a: float, b: float, ctx: Context[ServerSession, None]) -> str:
    49 │    """Add two numbers together."""
    50 │    result = a + b
    51 │    await ctx.info(f"Adding {a} and {b} = {result}")  # <-- TRIGGERS LOGGING
    52 │    return f"{a} + {b} = {result}"

  The ctx.info() method internally sends a JSON-RPC notification over the stdio pipe:

     1 │{
     2 │  "jsonrpc": "2.0",
     3 │  "method": "notifications/message",
     4 │  "params": {
     5 │    "level": "info",
     6 │    "data": "Adding 5 and 3 = 8"
     7 │  }
     8 │}

  Note: This is a notification (no id field), not a request. The server doesn't wait for a response.

  ────────────────────────────────────────


  Step 3: MCP SDK Routes the Notification

  The ClientSession has an internal message loop reading from read_stream. When it receives "method": "notifications/message", the MCP SDK internally does:

     1 │# Inside MCP SDK (simplified)
     2 │class ClientSession:
     3 │    async def _handle_incoming_message(self, message):
     4 │        if message.method == "notifications/message":
     5 │            # Call the registered callback!
     6 │            await self._logging_callback(message.params)
     7 │            # No response needed - it's a notification


  ────────────────────────────────────────


  Step 4: Your Callback Executes

  Now _handle_logs runs:

   ch4/01_handle_logging/client.py lines 36-50

    36 │async def _handle_logs(self, params: LoggingMessageNotificationParams) -> None:
    37 │    """
    38 │    Enhanced log handler to demonstrate when the server-initiated logging
    39 │    callback is triggered.
    40 │    """
    41 │    # Print debug directly so it always shows
    42 │    print(f"(DEBUG) Logging callback triggered by server: level={params.level}, data={params.data!r}")
    43 │
    44 │    if params.level in ("info", "error", "critical", "alert", "emergency"):
    45 │        logger.info(
    46 │            f"Handled server log in callback: level={params.level} | message={params.data!r}"
    47 │        )
    48 │        print(f"[{params.level}] - {params.data}")
    49 │    else:
    50 │        print(f"(DEBUG) Server log received but not shown: level={params.level}")
  ...
  | Aspect        | Logging                | Sampling            | 
  |---------------|------------------------|---------------------|
  | Message Type  | Notification (one-way) | Request (two-way)   | 
  | Server Waits? | No                     | Yes                 | 
  | Return Value  | None                   | CreateMessageResult | 
  | JSON-RPC      | No id field            | Has id field        | 
  ...
  Visual Summary
     ...
     1 │SERVER PROCESS                          CLIENT PROCESS
     2 │──────────────                          ──────────────
     3 │
     4 │add(5, 3)
     5 │    │
     6 │    ▼
     7 │await ctx.info("Adding 5 and 3 = 8")
     8 │    │
     9 │    │  ┌─────────────────────────────┐
    10 │    │  │  JSON-RPC NOTIFICATION      │
    11 │    └──┤  method: notifications/msg  ├──▶ ClientSession._message_loop()
    12 │       │  params: {level, data}      │            │
    13 │       │  (no "id" - fire & forget)  │            ▼
    14 │       └─────────────────────────────┘   if method == "notifications/message":
    15 │                                             await self._logging_callback(params)
    16 │    │                                              │
    17 │    │  (server continues immediately,              ▼
    18 │    │   doesn't wait for response)       _handle_logs(params)
    19 │    │                                              │
    20 │    ▼                                              ▼
    21 │return f"5 + 3 = 8"                      print("[info] - Adding 5 and 3 = 8")
    22 │
    23 │                                         (no response sent back)
  ...
  The server can use these context methods:

     1 │await ctx.debug("...")     # level = "debug"
     2 │await ctx.info("...")      # level = "info"
     3 │await ctx.warning("...")   # level = "warning"
     4 │await ctx.error("...")     # level = "error"

  Each becomes params.level in your callback.
  ...
  When you run the agent and ask "add 5 and 3":

     1 │You: add 5 and 3
     2 │Using tool: add
     3 │(DEBUG) Logging callback triggered by server: level=info, data='Adding 5.0 and 3.0 = 8.0'
     4 │[info] - Adding 5.0 and 3.0 = 8.0
     5 │Assistant: 5 + 3 = 8

  The [info] line comes from your _handle_logs callback being triggered by the server's await ctx.info().
...
## How Sampling Works for Division by Zero

```html
  Looking at the code, there are two different tools involved:

  1. The `divide` Tool (No Sampling)


   ch4/calculator_server.py lines 156-161

   156 │@mcp.tool()
   157 │async def divide(a: float, b: float, ctx: Context[ServerSession, None]) -> str:
   158 │    if b == 0:
   159 │        return "Error: Division by zero is not allowed"
   160 │    result = a / b
   161 │    return f"{a} ÷ {b} = {result}"

  This tool does NOT use sampling - it just returns a static error message.

  ────────────────────────────────────────


  2. The `explain_math` Tool (Uses Sampling)


   ch4/calculator_server.py lines 157-175

   157 │@mcp.tool()
   158 │async def explain_math(operation: str, ctx: Context[ServerSession, None]) -> str:
   159 │    """Use sampling to explain how a mathematical operation works."""
   160 │    prompt = f"""
   161 │    Explain how the following mathematical operation works. Break it down into 
   162 │    discrete steps and explain any relevant concepts. The operation is: {operation}.
   163 │    Use the voice of a patient but eccentric math professor explaining to a curious
   164 │    but inexperienced student.
   165 │    """
   166 │
   167 │    result = await ctx.session.create_message(
   168 │        messages=[
   169 │            SamplingMessage(
   170 │                role="user",
   171 │                content=TextContent(type="text", text=prompt),
   172 │            )
   173 │        ],
   174 │        max_tokens=100,
   175 │    )

  This is where sampling happens via ctx.session.create_message().

  ────────────────────────────────────────


  The Flow for "Explain Division by Zero"


     1 │User: "Explain why I can't divide by zero"
     2 │
     3 │1. Agent (Claude) → selects `explain_math` tool
     4 │
     5 │2. Agent → calls MCPClient.use_tool("explain_math", {"operation": "division by zero"})
     6 │
     7 │3. MCPClient → sends request to Calculator Server
     8 │
     9 │4. Server (explain_math) → calls ctx.session.create_message(...)
    10 │   This triggers a SAMPLING REQUEST back to the client
    11 │
    12 │5. Client's _handle_sampling callback receives the request:
    13 │
    14 │   async def _handle_sampling(self, context, params):
    15 │       # Server wants LLM to explain "division by zero"
    16 │       # Client calls its own LLM:
    17 │       response = self._llm_client.messages.create(
    18 │           max_tokens=params.maxTokens,
    19 │           messages=messages,  # Contains the explanation prompt
    20 │           model="claude-sonnet-4-0",
    21 │       )
    22 │       return CreateMessageResult(...)
    23 │
    24 │6. Server receives the LLM explanation → returns it as tool result
    25 │
    26 │7. Agent receives explanation → presents to user
  ...
  Visual Flow
     ...
     1 │┌─────────┐     ┌───────────┐     ┌─────────────────┐     ┌─────────┐
     2 ││  User   │────▶│   Agent   │────▶│   Calculator    │────▶│  Claude │
     3 ││         │     │  (Claude) │     │    Server       │     │   LLM   │
     4 │└─────────┘     └───────────┘     └─────────────────┘     └─────────┘
     5 │                     │                    │                    │
     6 │                     │  1. use tool       │                    │
    ...
    12 │                     │◀───────────────────│                    │
    13 │                     │                    │                    │
    14 │                     │  3. _handle_sampling callback           │
    15 │                     │  calls Claude LLM  │                    │
    16 │                     │─────────────────────────────────────────▶
    17 │                     │                    │                    │
    18 │                     │◀─────────────────────────────────────────
  ...
  The agent should select explain_math, which will trigger the sampling callback and you'll see the server requesting an LLM explanation through your client.
  Note: If you just ask "divide 10 by 0", it will use the divide tool which returns the static error without sampling.
...
```html
(base) aelkhodary@Ahmeds-MacBook-Pro ai_agents_mcp_examples % uv run ch4/02_sampling_callback/agent.py 
Welcome to your AI Assistant. Type 'goodbye' to quit or 'refresh' to reload and redisplay available resources.
[02/08/26 21:46:57] INFO     Processing request of type ListToolsRequest                                                                                                                                                                                        server.py:674
...
You: Explain why division by zero is undefined
Using tool: explain_math
[02/08/26 21:47:15] INFO     Processing request of type CallToolRequest                                                                                                                                                                                         server.py:674
(DEBUG) Logging callback triggered by server: level=info, data='Sending math explanation to LLM'
[info] - Sending math explanation to LLM
Assistant: The explanation from our mathematical sampling shows that division by zero is indeed a fundamental concept that mathematicians take very seriously! Let me expand on this with a clear mathematical explanation:

**Why Division by Zero is Undefined:**

1. **Definition Problem**: Division is defined as the inverse of multiplication. When we say a ÷ b = c, we mean that b × c = a. But if b = 0, then we're looking for a number c such that 0 × c = a. 

2. **The Contradiction**: 
   - If a ≠ 0 (say a = 5), then we need 0 × c = 5. But zero times any number is always zero, never 5. So no solution exists.
   - If a = 0, then we need 0 × c = 0. But this is true for ANY value of c! So we'd have infinitely many "answers."

3. **Limit Behavior**: As we divide by numbers closer and closer to zero, the results grow without bound:
   - 1 ÷ 1 = 1
   - 1 ÷ 0.1 = 10
   - 1 ÷ 0.01 = 100
   - 1 ÷ 0.001 = 1000
   
   The result approaches infinity, but infinity isn't a real number we can work with in standard arithmetic.

4. **Mathematical Consistency**: If we allowed division by zero, it would break the logical consistency of mathematics and lead to contradictions where we could "prove" that any number equals any other number.

That's why mathematicians declare division by zero as **undefined** - it simply doesn't have a meaningful answer within our number system!  


```

## The Programmatic Connection Explained

```html
  

  Here's the exact code flow that connects explain_math to _handle_sampling:
  ...
  When client connects, it registers _handle_sampling with ClientSession:

   ch4/02_sampling_callback/client.py lines 121-132

   121 │async def connect(self) -> None:
   122 │    # ... setup code ...
   123 │    
   124 │    # Start MCP client session
   125 │    self._session = await self._exit_stack.enter_async_context(
   126 │        ClientSession(
   127 │            read_stream=self.read,
   128 │            write_stream=self.write,
   129 │            logging_callback=self._handle_logs,
   130 │            sampling_callback=self._handle_sampling,  # <-- REGISTERED HERE
   131 │        ),
   132 │    )

  The ClientSession internally stores this callback function reference.

  ────────────────────────────────────────


  Step 2: Server Calls `create_message()`

  When explain_math runs on the server:

   ch4/calculator_server.py lines 166-174

   166 │result = await ctx.session.create_message(
   167 │    messages=[
   168 │        SamplingMessage(
   169 │            role="user",
   170 │            content=TextContent(type="text", text=prompt),
   171 │        )
   172 │    ],
   173 │    max_tokens=100,
   174 │)

  This does NOT call the callback directly. Instead, it sends a JSON-RPC message over the stdio pipe:

     1 │{
     2 │  "jsonrpc": "2.0",
     3 │  "method": "sampling/createMessage",
     4 │  "params": {
     5 │    "messages": [{"role": "user", "content": {"type": "text", "text": "Explain..."}}],
     6 │    "maxTokens": 100
     7 │  },
     8 │  "id": 1
     9 │}
  ...
  Step 3: MCP SDK Routes the Message

  The ClientSession has an internal message loop reading from read_stream. When it receives "method": "sampling/createMessage", the MCP SDK internally does something like:

     1 │# Inside MCP SDK (simplified)
     2 │class ClientSession:
     3 │    async def _handle_incoming_message(self, message):
     4 │        if message.method == "sampling/createMessage":
     5 │            # Call the registered callback!
     6 │            result = await self._sampling_callback(context, message.params)
     7 │            # Send result back to server
     8 │            await self._send_response(message.id, result)
  ...
  Now _handle_sampling runs:

   ch4/02_sampling_callback/client.py lines 52-75

    52 │async def _handle_sampling(
    53 │    self,
    54 │    context: RequestContext[ClientSession, None],
    55 │    params: CreateMessageRequestParams,
    56 │) -> CreateMessageResult | ErrorData:
    57 │    # Convert messages
    58 │    messages = []
    59 │    for message in params.messages:
    60 │        if isinstance(message.content, TextContent):
    61 │            messages.append(
    62 │                {"role": message.role, "content": message.content.text}
    63 │            )
    64 │    
    65 │    # Call YOUR LLM
    66 │    response = self._llm_client.messages.create(
    67 │        max_tokens=params.maxTokens,
    68 │        messages=messages,
    69 │        model="claude-sonnet-4-0",
    70 │    )
    71 │    
    72 │    # Return result to server
    73 │    return CreateMessageResult(
    74 │        role=response.role, content=content_data, model="claude-sonnet-4-0"
    75 │    )
  ...
  Step 5: Result Returns to Server

  The CreateMessageResult is serialized as JSON-RPC response and sent back over the pipe. The server's await ctx.session.create_message() receives it and continues.
  ...
  Visual Summary
     ...
     5 │    │
     6 │    ▼
     7 │ctx.session.create_message()
     8 │    │
     9 │    │  ┌─────────────────────────────┐
    10 │    │  │  JSON-RPC over stdio pipe   │
    11 │    └──┤  method: sampling/create... ├──▶ ClientSession._message_loop()
    12 │       └─────────────────────────────┘            │
    13 │                                                  ▼
    14 │                                        if method == "sampling/createMessage":
    15 │                                            await self._sampling_callback(...)
    16 │                                                  │
    17 │                                                  ▼
    18 │                                        _handle_sampling()
    19 │                                            │
    20 │                                            ▼
    21 │                                        anthropic_client.messages.create()
    22 │                                            │
    23 │                                            ▼
    24 │                                        return CreateMessageResult(...)
    25 │       ┌─────────────────────────────┐            │
    26 │       │  JSON-RPC response          │◀───────────┘
    27 │    ┌──┤  result: {content: "..."}   │
    28 │    │  └─────────────────────────────┘
    29 │    ▼
    30 │result = await ctx.session.create_message()
    31 │    │
    32 │    ▼
    33 │return result.content.text
  ...
  There is no direct function call between processes. The connection is:
  1. Registration: ClientSession(sampling_callback=self._handle_sampling)
  2. Message: Server sends JSON-RPC request over pipe
  3. Routing: MCP SDK reads pipe, matches method name, invokes registered callback
  4. Response: Callback result sent back over pipe

  The MCP SDK handles all the message routing internally - you just register the callback and it gets called when the server requests sampling.
...
```html
    (base) aelkhodary@Ahmeds-MacBook-Pro ai_agents_mcp_examples % uv run ch4/03_providing_roots/agent.py  
Welcome to your AI Assistant. Type 'goodbye' to quit or 'refresh' to reload and redisplay available resources.
[02/08/26 23:12:10] INFO     Processing request of type ListToolsRequest                                                                                                                                                                                        server.py:674
                    INFO     Processing request of type ListResourcesRequest                                                                                                                                                                                    server.py:674
                    INFO     Processing request of type ListPromptsRequest                                                                                                                                                                                      server.py:674
Loaded 1 resources and 1 prompts
You: Count all Python files in my project
Assistant: I'd be happy to help you count the Python files in your project! However, I need to know the directory path where your project is located. Could you please provide the file path to your project directory?
...
Once you provide the path, I can count the files for you.
You: root file '/Users/aelkhodary/Documents/GitHub/ai_agents_mcp_examples/ch4/03_providing_roots'
Using tool: count_files
[02/08/26 23:14:23] INFO     Processing request of type CallToolRequest                                                                                                                                                                                         server.py:674
[info] - Counting files in /Users/aelkhodary/Documents/GitHub/ai_agents_mcp_examples/ch4/03_providing_roots = 3
Assistant: There are 3 files in the directory `/Users/aelkhodary/Documents/GitHub/ai_agents_mcp_examples/ch4/03_providing_roots`.

``` 
  ### The Roots Callback Flow Explained
  ...
  Step 1: Client Configures Allowed Roots

  The client is initialized with file_roots - the directories the server is allowed to access:

   ch4/03_providing_roots/client.py lines 29-39
    ...
    32 │        name: str,
    33 │        command: str,
    34 │        server_args: list[str],
    35 │        llm_client: Anthropic,
    36 │        env_vars: dict[str, str] = None,
    37 │        file_roots: list[str] = None,  # <-- ALLOWED PATHS
    38 │    ) -> None:
    39 │        self.file_roots = file_roots   # <-- STORED HERE

  Example initialization:

     1 │mcp_client = MCPClient(
     2 │    name="calculator",
     3 │    file_roots=["file:///Users/you/projects/safe_folder"]
     4 │)
  ...
  Step 2: Client Registers the Callback

  When client connects, it registers _handle_roots with ClientSession:

   ch4/03_providing_roots/client.py lines 142-151

   142 │# Start MCP client session
   143 │self._session = await self._exit_stack.enter_async_context(
   144 │    ClientSession(
   145 │        read_stream=self.read,
   146 │        write_stream=self.write,
   147 │        logging_callback=self._handle_logs,
   148 │        sampling_callback=self._handle_sampling,
   149 │        list_roots_callback=self._handle_roots,  # <-- REGISTERED HERE
   150 │    ),
   151 │)
  ...
  Step 3: Server Requests Roots

  When count_files tool runs, the server asks the client for allowed roots:

   ch4/calculator_server.py lines 238-242

   238 │@mcp.tool()
   239 │async def count_files(file_path: str, ctx: Context[ServerSession, None]) -> str:
   240 │    """Count files in a given directory."""
   241 │    roots_result = await ctx.session.list_roots()  # <-- TRIGGERS CALLBACK
   242 │    root_uris: list[FileUrl] = [root.uri for root in roots_result.roots]

  The ctx.session.list_roots() sends a JSON-RPC request over the stdio pipe:
  ...
  Step 4: MCP SDK Routes the Request

  The ClientSession receives the message and routes it to the registered callback:

     1 │# Inside MCP SDK (simplified)
     2 │class ClientSession:
     3 │    async def _handle_incoming_message(self, message):
     4 │        if message.method == "roots/list":
     5 │            # Call the registered callback!
     6 │            result = await self._list_roots_callback(context)
     7 │            # Send result back to server
     8 │            await self._send_response(message.id, result)
  ...
  Step 5: Your Callback Executes

  Now _handle_roots runs:

   ch4/03_providing_roots/client.py lines 105-120

   105 │async def _handle_roots(
   106 │    self,
   107 │    context: RequestContext[ClientSession, Any],
   108 │) -> ListRootsResult | ErrorData:
   109 │    """
   110 │    Roots handler that returns the file roots, implementing the RootsFnT protocol.
   111 │    """
   112 │    roots_result = []
   113 │    for root in self.file_roots:
   114 │        if not root.startswith("file:///"):
   115 │            logger.warning(f"Root {root} does not start with file:///, ignoring")
   116 │        else:
   117 │            roots_result.append(Root(uri=root))
   118 │    if roots_result is None:
   119 │        return ErrorData(code=-32602, message="No valid file roots provided")
   120 │    return ListRootsResult(roots=roots_result)


  ────────────────────────────────────────


  Step 6: Server Validates Access

  The server receives the roots and checks if the requested path is allowed:

   ch4/calculator_server.py lines 244-259

   244 │# Quick validation using string matching
   245 │file_path_abs = os.path.abspath(file_path)
   246 │is_allowed = False
   247 │
   248 │for root_uri in root_uris:
   249 │    absolute_root_path = os.path.abspath(root_uri.path)
   250 │    if file_path_abs.startswith(absolute_root_path):
   251 │        is_allowed = True
   252 │        break
   253 │
   254 │if not is_allowed:
   255 │    error_msg = (
   256 │        f"Access denied: {file_path} is not within allowed roots {root_uris}"
   257 │    )
   258 │    await ctx.error(error_msg)
  ...
  Visual Summary
     ...
     2 │──────────────                          ──────────────
     3 │
     4 │count_files("/some/path")
     5 │    │
     6 │    ▼
     7 │roots_result = await ctx.session.list_roots()
     8 │    │
     9 │    │  ┌─────────────────────────────┐
    10 │    │  │  JSON-RPC REQUEST           │
    11 │    └──┤  method: roots/list         ├──▶ ClientSession._message_loop()
    12 │       │  id: 1                      │            │
    13 │       └─────────────────────────────┘            ▼
    14 │                                        if method == "roots/list":
    15 │                                            result = await self._list_roots_callback(ctx)
    16 │                                                  │
    17 │                                                  ▼
    18 │                                        _handle_roots(context)
    19 │                                            │
    20 │                                            ▼
    21 │                                        for root in self.file_roots:
    22 │                                            roots_result.append(Root(uri=root))
    23 │                                            │
    24 │                                            ▼
    25 │                                        return ListRootsResult(roots=[
    26 │                                            Root(uri="file:///safe/path")
    27 │                                        ])
    28 │       ┌─────────────────────────────┐            │
    29 │       │  JSON-RPC RESPONSE          │◀───────────┘
    30 │    ┌──┤  id: 1                      │
    31 │    │  │  result: {roots: [...]}     │
    32 │    │  └─────────────────────────────┘
    33 │    ▼
    34 │roots_result = ListRootsResult(roots=[...])
    35 │    │
    36 │    ▼
    37 │# Check if "/some/path" is within allowed roots
    38 │if file_path_abs.startswith(root_path):
    39 │    is_allowed = True
    40 │    │
    41 │    ▼
    42 │# If allowed: count files
    43 │# If not allowed: raise ValueError("Access denied")
  ...
  | Aspect        | Logging         | Sampling            | Roots             | 
  |---------------|-----------------|---------------------|-------------------|
  | Direction     | Server → Client | Server → Client     | Server → Client   | 
  | Message Type  | Notification    | Request             | Request           | 
  | Server Waits? | No              | Yes                 | Yes               | 
  | Return Value  | None            | CreateMessageResult | ListRootsResult   | 
  | Purpose       | Debug info      | LLM access          | Security boundary | 
  ...
  The roots callback is advisory only. The server chooses to respect it:

     1 │# Server VOLUNTARILY checks roots
     2 │if not is_allowed:
     3 │    raise ValueError("Access denied")  # Server enforces this
     4 │
     5 │# A malicious server could ignore roots entirely!

  Your host application should enforce actual filesystem sandboxing if security is critical. The roots mechanism is for cooperative servers to know their boundaries.
  ...
  When you run the agent:
     ...
     1 │You: Count files in /Users/you/projects/safe_folder
     2 │Using tool: count_files
     3 │[info] - Counting files in /Users/you/projects/safe_folder = 15
     4 │Assistant: There are 15 files in /Users/you/projects/safe_folder
     5 │
     6 │You: Count files in /etc/secrets
     7 │Using tool: count_files
     8 │[error] - Access denied: /etc/secrets is not within allowed roots [...]
     9 │Assistant: Access denied. That path is outside the allowed directories.

```



## 4. Returning Elicitations 
  ```html

  (`04_returning_elicitations/`)
  ...
  ```html
   ...
   The Elicitation Callback Flow Explained

  Elicitation allows the server to request structured user input (forms, confirmations) through the client.

  ────────────────────────────────────────


  Step 1: Server Defines the Form Schema

  The server defines what data it needs using JSON Schema:

   ch4/calculator_server.py lines 18-43

    18 │# Form schema for elicitation requests
    19 │FORM_SCHEMA = {
    20 │    "type": "object",
    21 │    "properties": {
    22 │        "name": {
    23 │            "type": "string",
    24 │            "title": "Full Name",
    25 │            "description": "Your full name",
    26 │            "minLength": 1,
    27 │        },
    28 │        "email": {
    29 │            "type": "string",
    30 │            "title": "Email Address",
    31 │            "description": "Your email address",
    32 │            "format": "email",
    33 │        },
    34 │        "age": {
    35 │            "type": "number",
    36 │            "title": "Age",
    37 │            "description": "Your age in years",
    38 │            "minimum": 0,
    39 │            "maximum": 150,
    40 │        },
    41 │    },
    42 │    "required": ["name", "email"],
    43 │}
  ...
  When client connects, it registers _handle_elicitation with ClientSession:

   ch4/04_returning_elicitations/client.py lines 286-296

   286 │# Start MCP client session
   287 │self._session = await self._exit_stack.enter_async_context(
   288 │    ClientSession(
   289 │        read_stream=self.read,
   290 │        write_stream=self.write,
   291 │        logging_callback=self._handle_logs,
   292 │        sampling_callback=self._handle_sampling,
   293 │        list_roots_callback=self._handle_roots,
   294 │        elicitation_callback=self._handle_elicitation,  # <-- REGISTERED HERE
   295 │    ),
   296 │)
  ...
  Step 3: Server Requests User Input

  When signup_math_facts tool runs, the server asks the client to collect user data:

   ch4/calculator_server.py lines 186-196

   186 │@mcp.tool()
   187 │async def signup_math_facts(ctx: Context[ServerSession, None]) -> str:
   188 │    """Sign up to receive daily math facts (demonstration of elicitation)."""
   189 │    # Make elicitation request to collect user information
   190 │    try:
   191 │        elicit_result = await ctx.session.elicit(  # <-- TRIGGERS CALLBACK
   192 │            message="Please provide your information to sign up for daily math facts!",
   193 │            requestedSchema=FORM_SCHEMA,
   194 │        )
   195 │    except Exception as e:
   196 │        return f"Sorry, there was an error during signup: {str(e)}"

  The ctx.session.elicit() sends a JSON-RPC request over the stdio pipe:

     1 │{
     2 │  "jsonrpc": "2.0",
     3 │  "method": "elicitation/create",
     4 │  "params": {
     5 │    "message": "Please provide your information to sign up for daily math facts!",
     6 │    "requestedSchema": {
     7 │      "type": "object",
     8 │      "properties": {
     9 │        "name": {"type": "string", "description": "Your full name"},
    10 │        "email": {"type": "string", "description": "Your email address"},
    11 │        "age": {"type": "number", "description": "Your age in years"}
    12 │      },
  ...
  The ClientSession receives the message and routes it to the registered callback:
     ...
     2 │class ClientSession:
     3 │    async def _handle_incoming_message(self, message):
     4 │        if message.method == "elicitation/create":
     5 │            # Call the registered callback!
     6 │            result = await self._elicitation_callback(context, message.params)
     7 │            # Send result back to server
     8 │            await self._send_response(message.id, result)
  ...
  Now _handle_elicitation runs. It has 3 phases:
  Phase A: Display the request to user

   ch4/04_returning_elicitations/client.py lines 217-230

   217 │async def _handle_elicitation(
   218 │    self,
   219 │    context: RequestContext[ClientSession, Any],
   220 │    params: ElicitRequestParams,
   221 │) -> ElicitResult | ErrorData:
   222 │    # Get the server name from the client instance
   223 │    requesting_server = self.name
   224 │
   225 │    # Display the elicitation request to the user
   226 │    print(f"\n{'='*60}")
   227 │    print(f"ELICITATION REQUEST FROM SERVER: {requesting_server}")
   228 │    print(f"{'='*60}")
   229 │    print(f"Message: {params.message}")
   230 │    print(f"{'='*60}")

  Phase B: Get user decision (accept/decline/cancel)

   ch4/04_returning_elicitations/client.py lines 237-260

   237 │    # Get user input for accept/decline
   238 │    while True:
   239 │        user_response = (
   240 │            input("\nDo you want to accept this request? (y/n/c for cancel): ")
   241 │            .lower()
   ...
   243 │        )
   244 │
   245 │        if user_response in ["y", "yes", "accept"]:
   246 │            print("Request accepted")
   247 │            # Collect form data based on the schema
   248 │            form_data = self._collect_form_data(params.requestedSchema)
   249 │            if form_data is not None:
   250 │                print("Form data collected successfully")
   251 │                return ElicitResult(action="accept", content=form_data)
   252 │            else:
   253 │                print("Form data collection cancelled")
   254 │                return ElicitResult(action="cancel")
   255 │        elif user_response in ["n", "no", "decline"]:
   256 │            print("Request declined")
   257 │            return ElicitResult(action="decline")
   258 │        elif user_response in ["c", "cancel"]:
   259 │            print("Request cancelled")
   260 │            return ElicitResult(action="cancel")

  Phase C: Collect form data (if accepted)

   ch4/04_returning_elicitations/client.py lines 125-142

   125 │def _collect_form_data(self, schema: dict[str, Any]) -> dict[str, Any] | None:
   126 │    print(f"\n{'='*60}")
   127 │    print("FORM DATA REQUIRED")
   128 │    print(f"{'='*60}")
   129 │
   130 │    # Display schema information
   131 │    if "properties" in schema:
   132 │        print("Required fields:")
   133 │        for field_name, field_info in schema["properties"].items():
   134 │            field_type = field_info.get("type", "string")
   135 │            description = field_info.get("description", "")
   136 │            required = field_name in schema.get("required", [])
   137 │            required_text = " (required)" if required else " (optional)"
   138 │            print(f"  • {field_name} ({field_type}){required_text}: {description}")
   139 │    
   140 │    # ... collect each field with validation and type conversion ...
   141 │    
   142 │    return collected_data


  ────────────────────────────────────────


  Step 6: Server Handles the Response

  The server receives the ElicitResult and processes it:

   ch4/calculator_server.py lines 201-221

   201 │    # Handle the different response actions
   202 │    if elicit_result.action == "accept":
   203 │        user_data = elicit_result.content
   204 │        await ctx.info(f"User signed up: {user_data}")
   205 │
   206 │        # Extract user information
   207 │        name = user_data["name"]
   208 │        email = user_data["email"]
   209 │        age = user_data.get("age")
   210 │
   211 │        response = f"Welcome {name}! You've successfully signed up...\n"
   212 │        response += f"We'll send interesting mathematical tidbits to {email}.\n"
   213 │        return response
   214 │
   215 │    elif elicit_result.action == "decline":
   216 │        await ctx.info("User declined math facts signup")
   217 │        return "No problem! You've chosen not to sign up..."
   218 │
   219 │    elif elicit_result.action == "cancel":
   220 │        await ctx.info("User cancelled math facts signup")
   221 │        return "Signup cancelled. Feel free to try again later..."
  ...
  Visual Summary
     ...
     5 │    │
     6 │    ▼
     7 │elicit_result = await ctx.session.elicit(
     8 │    message="Please provide your info...",
     9 │    requestedSchema=FORM_SCHEMA
    10 │)
    11 │    │  ┌─────────────────────────────┐
    12 │    │  │  JSON-RPC REQUEST           │
    13 │    └──┤  method: elicitation/create ├──▶ ClientSession._message_loop()
    14 │       │  params: {message, schema}  │            │
    15 │       │  id: 1                      │            ▼
    16 │       └─────────────────────────────┘   if method == "elicitation/create":
    17 │                                             result = await self._elicitation_callback(ctx, params)
    18 │                                                  │
    19 │                                                  ▼
    20 │                                        _handle_elicitation(context, params)
    21 │                                            │
    22 │                                            ▼
    23 │                                        ┌─────────────────────────────────┐
    24 │                                        │  TERMINAL INTERACTION           │
    25 │                                        │  ════════════════════════════   │
    26 │                                        │  ELICITATION REQUEST FROM...    │
    27 │                                        │  Message: Please provide...     │
    28 │                                        │  ════════════════════════════   │
    29 │                                        │                                 │
    30 │                                        │  Accept? (y/n/c): y             │
    31 │                                        │                                 │
    32 │                                        │  FORM DATA REQUIRED             │
    33 │                                        │  • name (string) (required)     │
    34 │                                        │  • email (string) (required)    │
    35 │                                        │  • age (number) (optional)      │
    36 │                                        │                                 │
    37 │                                        │  Enter name: John Doe           │
    38 │                                        │  Enter email: john@example.com  │
    39 │                                        │  Enter age: 30                  │
    40 │                                        └─────────────────────────────────┘
    41 │                                            │
    42 │                                            ▼
    43 │                                        return ElicitResult(
    44 │                                            action="accept",
    45 │                                            content={
    46 │                                                "name": "John Doe",
    ...
    50 │                                        )
    51 │       ┌─────────────────────────────┐            │
    52 │       │  JSON-RPC RESPONSE          │◀───────────┘
    53 │    ┌──┤  id: 1                      │
    54 │    │  │  result: {action, content}  │
    55 │    │  └─────────────────────────────┘
    56 │    ▼
    57 │elicit_result.action == "accept"
    58 │elicit_result.content == {"name": "John Doe", ...}
    59 │    │
    60 │    ▼
    61 │return "Welcome John Doe! You've successfully signed up..."
  ...
  | Action  | Meaning            | ElicitResult                                 | Use Case                         | 
  |---------|--------------------|----------------------------------------------|----------------------------------|
  | accept  | User provided data | ElicitResult(action="accept", content={...}) | Form completed                   | 
  | decline | User said no       | ElicitResult(action="decline")               | User doesn't want to proceed     | 
  | cancel  | User aborted       | ElicitResult(action="cancel")                | User changed their mind mid-form | 
  ...
  | Aspect           | Logging         | Sampling            | Roots           | Elicitation     | 
  |------------------|-----------------|---------------------|-----------------|-----------------|
  | Direction        | Server → Client | Server → Client     | Server → Client | Server → Client | 
  | Message Type     | Notification    | Request             | Request         | Request         | 
  | User Interaction | No              | No                  | No              | Yes             | 
  | Return Value     | None            | CreateMessageResult | ListRootsResult | ElicitResult    | 
  | Purpose          | Debug info      | LLM access          | Security        | User input      | 
  ...
  Example Output
     ...
     1 │You: Sign me up for math facts
     2 │
     3 │Using tool: signup_math_facts
     4 │
     5 │============================================================
     6 │ELICITATION REQUEST FROM SERVER: calculator_server_connection
     7 │============================================================
     8 │Message: Please provide your information to sign up for daily math facts!
     9 │============================================================
    10 │
    11 │Do you want to accept this request? (y/n/c for cancel): y
    12 │Request accepted
    13 │
    14 │============================================================
    15 │FORM DATA REQUIRED
    16 │============================================================
    17 │Required fields:
    18 │  • name (string) (required): Your full name
    19 │  • email (string) (required): Your email address
    20 │  • age (number) (optional): Your age in years
    21 │============================================================
    22 │
    23 │Enter name (Your full name): John Doe
    24 │
    25 │Enter email (Your email address): john@example.com
    26 │
    27 │Enter age (Your age in years) [optional]: 30
    28 │
    29 │Form data collected successfully
    30 │[info] - User signed up: {'name': 'John Doe', 'email': 'john@example.com', 'age': 30}
